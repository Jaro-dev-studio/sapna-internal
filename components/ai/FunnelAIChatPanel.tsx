...
interface FunnelAIChatPanelProps {
  isOpen: boolean;
  onClose: () => void;
  flowData: any;
}

export function FunnelAIChatPanel({ isOpen, onClose, flowData }: FunnelAIChatPanelProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("/api/chat/funnel", {
        method: "POST",
  ...
  return (
    <motion.div
      ...
      className="fixed right-0 top-0 z-50 flex h-full w-[600px] flex-col bg-background p-4 shadow-lg dark:bg-background-dark"
    >
      ...
      <div className="flex gap-2 border-t border-border pt-4 dark:border-border-dark">
        <input
          ...
          placeholder="Ask about your funnel metrics..."
